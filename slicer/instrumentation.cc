...
void GenerateShiftParamsCode(lir::CodeIr* code_ir, lir::Instruction* position,
  ...
  // Since the goal is to relocate the registers when extra scratch registers
  // are needed, if there are no parameters this is a no-op.
...
bool EntryHook::InjectArrayParamsHook(lir::CodeIr* code_ir,
                                      lir::Bytecode* bytecode) {
  ir::Builder builder(code_ir->dex_ir);
  const auto ir_method = code_ir->ir_method;
...
bool ExitHook::Apply(lir::CodeIr* code_ir) {
  ...
  bool return_void =
      (::strcmp(declared_return_type->descriptor->c_str(), "V") == 0);
  // returnAsObject supports only object return type;
  SLICER_CHECK(!return_as_object || (declared_return_type->GetCategory() ==
                                     ir::Type::Category::Reference));
  const auto return_type = return_as_object
  ...
  auto ir_proto =
      builder.GetProto(return_type, builder.GetTypeList(param_types));
  ...
  for (auto instr : code_ir->instructions) {
    ...
    if (pass_method_signature) {
      ...
      if (reg == 0 && bytecode->opcode != dex::OP_RETURN_VOID) {
        ...
        // return is the last call, return is shifted to one, so taking over 0
        // registry
        scratch_reg = 0;
      } else {
      ...
      const_str_op->opcode = dex::OP_CONST_STRING;
      const_str_op->operands.push_back(
          code_ir->Alloc<lir::VReg>(scratch_reg));  // dst
      const_str_op->operands.push_back(code_ir->Alloc<lir::String>(
          method_label, method_label->orig_index));  // src
      code_ir->instructions.InsertBefore(bytecode, const_str_op);
    }
    ...
    if (move_result_opcode != dex::OP_NOP) {
      ...
      if ((tweak_ & Tweak::ReturnAsObject) != 0) {
        ...
        check_cast->operands.push_back(code_ir->Alloc<lir::Type>(
...
// into their original registers (parameters are allocated in the last IN
// registers)
//
...
void AllocateScratchRegs::Allocate(lir::CodeIr* code_ir, dex::u4 first_reg,
                                   int count) {
...
// 2. if the method uses less than 16 registers, we can renumber the existing
// registers
// 3. if we still have registers to allocate, increase the method registers
// count,
//     and generate prologue code to shift the param regs into their original
//     registers
//
