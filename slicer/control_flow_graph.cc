...
std::vector<BasicBlock> BasicBlocksVisitor::Finish() {
  ...
  //  have labels, annotations, directives, etc. For example it's possible to
  //  have debug annotations (.line, .endlocal, ...) after the last bytecode.
  //
...
bool BasicBlocksVisitor::Visit(Bytecode* bytecode) {
  ...
  if (model_exceptions_) {
    constexpr auto exit_instr_flags =
        dex::kBranch | dex::kSwitch | dex::kThrow | dex::kReturn;
    terminate_block = (flags & exit_instr_flags) != 0;
  } else {
    constexpr auto exit_instr_flags =
        dex::kBranch | dex::kSwitch | dex::kReturn;
    terminate_block =
        bytecode->opcode == dex::OP_THROW || (flags & exit_instr_flags) != 0;
  }
  if (terminate_block) {
    EndBlock(bytecode);
  }
