...
// (currently a simple fail-fast but the the intention is to allow
// customization)
void _checkFailed(const char* expr, int line, const char* file)
    __attribute__((noreturn));
#define SLICER_CHECK(expr)                                        \
  do {                                                            \
    if (!(expr)) slicer::_checkFailed(#expr, __LINE__, __FILE__); \
  } while (false)
...
// Macros that check the binary relation between two values. If the relation is
// not true, the values are logged and the process aborts.
...
//
// NOTE: we use SLICER_WEAK_CHECK for .dex format validations that are
// frequently
...
#define SLICER_WEAK_CHECK(expr)                                       \
  do {                                                                \
    if (!(expr)) slicer::_weakCheckFailed(#expr, __LINE__, __FILE__); \
  } while (false)
...
#undef SLICER_CHECK
#define SLICER_CHECK(...)
#undef SLICER_CHECK_OP
#define SLICER_CHECK_OP(...)
#undef SLICER_WEAK_CHECK
#define SLICER_WEAK_CHECK(...)
#endif
