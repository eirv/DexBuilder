...
class ExitHook : public Transformation {
 public:
  enum class Tweak {
    ...
    // This can be helpful when the code you want to handle the hook doesn't
    // have access to the actual type in its classpath or when you want to
    // inject the same hook in multiple methods.
    ReturnAsObject = 1 << 0,
  ...
  explicit ExitHook(const ir::MethodId& hook_method_id)
      : ExitHook(hook_method_id, Tweak::None) {}
...
inline ExitHook::Tweak operator|(ExitHook::Tweak a, ExitHook::Tweak b) {
  return static_cast<ExitHook::Tweak>(static_cast<int>(a) |
                                      static_cast<int>(b));
}
...
class AllocateScratchRegs : public Transformation {
 public:
  explicit AllocateScratchRegs(int allocate_count,
                               bool allow_renumbering = true)
...
//    mi.AddTransformation<slicer::EntryHook>(ir::MethodId("LTracer;",
//    "OnEntry"));
//    mi.AddTransformation<slicer::ExitHook>(ir::MethodId("LTracer;",
//    "OnExit")); SLICER_CHECK(mi.InstrumentMethod(ir::MethodId("LHello;",
//    "Test", "(I)I")));
//    ...
//
class MethodInstrumenter {
 public:
  explicit MethodInstrumenter(std::shared_ptr<ir::DexFile> dex_ir)
      : dex_ir_(dex_ir) {}
