...
class BasicBlocksVisitor : public Visitor {
 ...
 public:
  explicit BasicBlocksVisitor(bool model_exceptions)
      : model_exceptions_(model_exceptions) {
    current_block_.id = 0;
  }

  ~BasicBlocksVisitor() { assert(state_ == State::Outside); }
 ...
 private:
  ...
  bool Visit(DbgInfoHeader* dbg_header) override {
    return HandleAnnotation(dbg_header);
  }
  bool Visit(DbgInfoAnnotation* dbg_annotation) override {
    return HandleAnnotation(dbg_annotation);
  }

  // EH annotations
  bool Visit(TryBlockBegin* try_begin) override {
    return SkipInstruction(try_begin);
  }
  bool Visit(TryBlockEnd* try_end) override { return SkipInstruction(try_end); }

  // data payload
  bool Visit(PackedSwitchPayload* packed_switch) override {
    return SkipInstruction(packed_switch);
  }
  bool Visit(SparseSwitchPayload* sparse_switch) override {
    return SkipInstruction(sparse_switch);
  }
  bool Visit(ArrayData* array_data) override {
    return SkipInstruction(array_data);
  }
...
struct ControlFlowGraph {
 ...
 public:
  ControlFlowGraph(const CodeIr* code_ir, bool model_exceptions)
      : code_ir(code_ir) {
