...
// Trim a 16bit signed integer, making sure we're not discarding significant
// bits
...
// (register fields can encode either a single 32bit vreg or a wide 64bit vreg
// pair)
...
bool BytecodeEncoder::Visit(Bytecode* bytecode) {
  ...
  SLICER_CHECK_EQ(bytecode_.size() - buff_offset,
                  2 * GetWidthFromFormat(format));
...
void BytecodeEncoder::Encode(ir::Code* ir_code,
