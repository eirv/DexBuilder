...
void CodeIr::FixupPackedSwitch(PackedSwitchPayload* instr, dex::u4 base_offset,
  ...
  auto dex_packed_switch =
      reinterpret_cast<const dex::PackedSwitchPayload*>(ptr);
...
void CodeIr::FixupSparseSwitch(SparseSwitchPayload* instr, dex::u4 base_offset,
  ...
  auto dex_sparse_switch =
      reinterpret_cast<const dex::SparseSwitchPayload*>(ptr);
...
Bytecode* CodeIr::DecodeBytecode(const dex::u2* ptr, dex::u4 offset) {
  ...
  switch (format) {
      ...
      switch (dex_instr.opcode) {
        ...
        case dex::OP_CONST_WIDE_HIGH16:
          ...
          instr->operands.push_back(
              Alloc<Const64>(dex::u8(dex_instr.vB) << 48));
          break;
    ...
    default: {
      slicer::StringStream ss;
      ss << "Unexpected bytecode format " << format << " for opcode "
         << dex_instr.opcode;
      SLICER_FATAL(ss.str());
    }
  }
...
IndexedOperand* CodeIr::GetIndexedOperand(dex::InstructionIndexType index_type,
  ...
  switch (index_type) {
    ...
    default:
      SLICER_FATAL(
          slicer::FormatString("Unexpected index type %04#x", index_type));
  }
}

// Get the second indexed object (if any).
IndexedOperand* CodeIr::GetSecondIndexedOperand(
    dex::InstructionIndexType index_type, dex::u4 index) {
  SLICER_CHECK_NE(index, dex::kNoIndex);
...
String* CodeIr::GetString(dex::u4 index) {
  auto ir_string =
      (index == dex::kNoIndex) ? nullptr : dex_ir->strings_map[index];
